...
Scenarios (`bash scripts/qa/run-all.sh <url> <outdir>` runs them all): `qa/full-playthrough.cjs` plays the whole game with real input only and never calls `solve`; `e2e-flow.cjs` covers the progression loop; `qa/terminal.cjs`, `qa/pipeline.cjs`, `qa/blocks.cjs`, `qa/keypad.cjs` and `qa/lanterns.cjs` cover one chamber each; `qa/ui.cjs` covers the desktop panels and a phone-sized joystick run; `qa/reset-flow.cjs` covers replaying after a progress reset; `qa/edge-cases.cjs` covers impatient input; `qa/visuals.cjs` takes screenshots in normal and lite modes. Each prints PASS/FAIL lines and exits non-zero on failure or console errors. Software rendering is slow; the scripts wait on state rather than fixed timers.

### Things that bit us once

The World stays mounted for the life of the page, so **anything that latches on `solved` must also unlatch** when progress is reset (`BlocksPuzzle` freezes crates, `Vault` plays its opening sequence, `Door` tracks whether it has opened). Overlay puzzles must **bank a solve on unmount** rather than only at the end of a success animation, since the player can close the overlay mid-animation. Deferred work scheduled after a solve should be **keyed to the overlay object that scheduled it** — `AnimatePresence` can reuse a same-keyed component across a close/reopen, so an unmount cleanup alone will not cancel it.
